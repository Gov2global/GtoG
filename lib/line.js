...
// --------- Lazy axios instance (อ่าน ENV ตอนเรียกใช้จริงเท่านั้น) ----------
let _lineAxios = null;
function getLineAxios() {
  const token = process.env.LINE_CHANNEL_ACCESS_TOKEN;
  if (!token) {
    // โยน error ตอนเรียกใช้ ไม่ใช่ตอน import → ปลอดภัยกับขั้นตอน build
    throw new Error("Missing LINE_CHANNEL_ACCESS_TOKEN env.");
  }

  // ถ้ายังไม่เคยสร้าง หรือ token เปลี่ยน ให้สร้างใหม่
  if (
    !_lineAxios ||
