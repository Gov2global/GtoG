...
export async function connectMongoDB() {
  // ถ้ามี connection ใช้ได้เลย
  if (cached.conn && mongoose.connection.readyState === 1) {
    return cached.conn;
  }

  // ถ้าเคยเริ่ม connect แล้ว (state=2) → รอให้เสร็จ
  if (cached.promise && mongoose.connection.readyState === 2) {
    return cached.promise;
  }

  // ถ้าอยู่ในสถานะ disconnecting (3) → รอให้ disconnect เสร็จก่อน
  if (mongoose.connection.readyState === 3) {
    await mongoose.disconnect();
  }

  // ถ้าไม่มี promise → connect ใหม่
  if (!cached.promise) {
    cached.promise = mongoose
      .connect(MONGODB_URI, {
        maxPoolSize: 20, // แนะนำลดจาก 100 → 20 ถ้าใช้บน Vercel (serverless)
        minPoolSize: 0,
      ...
      .catch((err) => {
        cached.promise = null; // reset ให้ connect ใหม่ได้ถ้า fail
        throw err;
      });
  }
